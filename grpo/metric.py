    ...
    try:
        ...
        if model is not None and tokenizer is not None:
            try:
                ...
                # 더 나은 정규화: PPL을 0-1 범위로 변환
                # PPL 1-10: 좋음(0.0), PPL 100+: 나쁨(1.0)
                ppl_normalized = min(1.0, max(0.0, (np.log(ppl) - np.log(1.0)) / (np.log(100.0) - np.log(1.0))))
                ppl_penalty = ppl_normalized
            except Exception as e:
                print_log(f"Error computing PPL penalty: {e}")
                ppl_penalty = 0.5  # Default penalty
        
        # Compute final reward with improved scaling
        # 각 컴포넌트를 [-1, 1] 범위로 정규화
        diversity_reward = -interactive_bleu  # 낮은 BLEU = 높은 다양성 보상
        quality_reward = rouge_l  # ROUGE-L 자체가 이미 0-1 범위
        fluency_reward = -ppl_penalty  # 낮은 PPL = 높은 유창성 보상
        
        reward_b = (
            lambda1 * diversity_reward +
            lambda2 * quality_reward +
            lambda3 * fluency_reward
        )
        
        # Length bonus for reasonable outputs
        gen_len = len(generated.split())
        if 5 <= gen_len <= 50:  # Reasonable length range
            reward_b += 0.1
        elif gen_len < 3:  # Too short penalty
            reward_b -= 0.2
        elif gen_len > 80:  # Too long penalty
            reward_b -= 0.1
        
        # Final scaling - GRPO에서는 너무 극단적인 값 방지
        reward_b = np.tanh(reward_b)  # [-1, 1] 범위로 부드럽게 제한
        
        return float(reward_b)
        
    except Exception as e:
        print_log(f"Error computing Adapter B reward: {e}")
        return -0.5  # 에러 시 약간의 패널티
